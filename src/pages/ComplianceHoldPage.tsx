...
export default function ComplianceHoldPage({ onLogout, user }: ComplianceHoldPageProps) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#0B0F17] p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-2xl bg-[#121824] rounded-xl border-2 border-red-500/20 shadow-2xl p-8 sm:p-12 relative z-10 text-center">
        {/* Alerts / Icon badge */}
        ...
        <p className="text-[#F59E0B] font-bold uppercase tracking-[0.2em] text-xs mt-2">
          Jurisdictional Anti-Fraud Clearance & Verification File Required
        </p>

        {user.customError && (
          <div className="max-w-md mx-auto bg-red-950/50 border border-red-500/30 p-5.5 rounded text-left mt-4 text-xs font-semibold text-red-300 flex flex-col gap-4 animate-bounce">
            ...
            <button 
              ...
              className="w-full px-4 py-2 bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30 rounded text-[9px] font-black uppercase tracking-widest transition-colors"
            >
        ...
        {/* Informative Grid */}
        <div className="bg-[#0B0F17]/60 rounded-xl p-6 my-8 text-left border border-[#1E2638] space-y-6">
          ...
          <div className="space-y-3 text-xs text-[#8E9BAE] leading-relaxed">
            <p>
              Your registration application was successfully received under account identifier <span className="font-mono text-[#F59E0B] font-bold">{user.accountNumber}</span>. However, to maintain alignment with cross-border banking guidelines (including the Bank of Papua New Guinea compliance channels, the Bank of England clearing regulations, and US federal AML standards), all newly self-registered digital bank portals are placed on <span className="text-red-400 font-bold">Compliance Hold</span>.
            </p>
            <p>
          ...
          <div className="pt-4 border-t border-[#1E2638] grid grid-cols-1 sm:grid-cols-3 gap-6 text-center">
            <div className="bg-[#121824]/40 p-5 rounded border border-[#1E2638]">
              <span className="block text-[8px] font-bold text-[#8E9BAE] uppercase tracking-widest">KYC Pipeline</span>
            ...
            <div className="bg-[#121824]/40 p-5 rounded border border-[#1E2638]">
              ...
              <span className="text-[#F59E0B] text-xs font-bold uppercase mt-1 block">{user.currency || 'USD'}</span>
            </div>
            <div className="bg-[#121824]/40 p-5 rounded border border-[#1E2638]">
              <span className="block text-[8px] font-bold text-[#8E9BAE] uppercase tracking-widest">Jurisdictions</span>
        ...
        <div className="space-y-6">
          <p className="text-xs text-[#8E9BAE] max-w-lg mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <button
               onClick={handleOpenSupport}
              className="w-full sm:w-auto px-6 py-3 bg-[#F59E0B] text-[#0B0F17] font-bold text-xs uppercase tracking-widest hover:bg-[#FF9500] transition-all flex items-center justify-center gap-3 shadow-lg border border-[#1E2638]"
            >
            ...
            <button
              onClick={onLogout}
              className="w-full sm:w-auto px-6 py-3 border border-[#1E2638] text-[#8E9BAE] font-bold text-xs uppercase tracking-widest hover:bg-[#121824] transition-all flex items-center justify-center gap-3"
            >
              <Lock size={16} />
              Secure Log Out
            </button>
        ...
        <div className="mt-8 text-[9px] text-[#8E9BAE] uppercase tracking-wider flex items-center justify-center gap-1.5">
          <ShieldAlert size={12} className="text-[#F59E0B]" /> Continuous 256-bit monitoring active on user portal
        </div>
      </div>
    </div>
  );
